...
import xyz.ring2.admin.core.mapper.CourseCategoryMapper;
 ...
 **/
@Service
public class GoodsCategoryServiceImpl extends ServiceImpl<CourseCategoryMapper, CourseCategory> implements GoodsCategoryService {
     ...
     */
    @Override
    public Map<String, Object> selGoodsCategoryByPage(Page<CourseCategory> page, Integer type) {
        ...
        if (page == null) {
            List<CourseCategory> goodsCategories = this.baseMapper.selectList(queryWrapper);
            map.put("data", getGoodsCategoryTree(goodsCategories, type));
        } else {
            IPage<CourseCategory> e = this.baseMapper.selectPage(page, queryWrapper);
            List<CourseCategory> records = e.getRecords();
            // 如果type=1 则无需获取子节点
            ...
            originGoodsCate.forEach(
                    goodsCategory -> {  // 对每个分类进行遍历
                        ...
                        if (type != null && type.equals(2)) {
                            children = lambdaQuery().eq(CourseCategory::getCatePid, goodsCategory.getId()).le(CourseCategory::getCateLevel, 1).list();
                        } else { // 如果type=3 或则为null则获取所有的子节点
                            children = lambdaQuery().eq(CourseCategory::getCatePid, goodsCategory.getId()).list();
                        }
                        // 每个子分类再进行递归调用获取其子分类
